    ...
    def start_game(self):
        while self.playing:
            ...
            if user_choice == "1":
                ...
                self.displayed_word = functions.initialize_displayed_word(
                    self.selected_word)
                time.sleep(1)
        ...
        while self.win_loss == 0:
            ...
            if guess in self.selected_word:
                ...
                if self.incorrect_guesses == self.max_incorrect_guesses:
                    functions.failed_game(self.selected_word)
                    functions.reset(self)
                    break
                else:
                    functions.incorrect_guess(self.incorrect_guesses)
            if "_" not in self.displayed_word:
                functions.won_game(self.displayed_word)
                functions.reset(self)
